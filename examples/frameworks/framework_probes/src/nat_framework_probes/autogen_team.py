...
# From NAT's nat_autogen_demo (Apache-2.0), no longer installed; max_tool_iterations made a field.
import logging
...
async def autogen_team(config: AutoGenFunctionConfig, builder: Builder) -> AsyncIterator[FunctionInfo]:
    """A researcher/finalizer pair in an AutoGen RoundRobinGroupChat, ended by 'APPROVE'."""
    ...
    try:
        ...
        async def _autogen_team_workflow(user_input: str) -> str:
            """One fresh team per invocation: a reused one raises "team is already running"."""
            try:
