...
"""One minimal single-agent workflow per framework, all with the same interface.

The shipped examples are task-specific (weather, traffic, travel), so none of them can run
a benchmark dataset; these take llm_name + tool_names and nothing else.
"""
