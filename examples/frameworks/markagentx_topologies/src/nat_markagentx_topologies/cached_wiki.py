...
"""Wikipedia search for benchmark sweeps: two API calls per query, cached on disk.

The `wikipedia` package spends ~7 requests per query and trips Wikipedia's 429 within a
handful of calls; batching the content fetch and caching keeps a full sweep inside limits.
"""
...
# A miss is often a throttled or half-served response, so it expires instead of poisoning the
# query for every later run; a real miss still costs one search per day.
...
async def wiki_search_cached(tool_config: CachedWikiSearchConfig, builder: Builder):
    ...
    async def _search(question: str) -> str:
        # An agent that pastes an article back in as the query builds a URL Wikipedia answers with
        # 414; normalising before the key also keeps whitespace variants on one cache entry.
