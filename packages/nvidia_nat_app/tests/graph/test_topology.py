...
class TestDetectCycles:
    ...
    def test_simple_cycle(self, cycle_graph):
        cycles = detect_cycles(cycle_graph)
        assert len(cycles) == 1
        assert len(cycles[0].nodes) == 3

    def test_cycle_entry_and_exit(self, cycle_graph):
        cycles = detect_cycles(cycle_graph)
        c = cycles[0]
...
class TestCycleNodeOrder:
    ...
    def test_order_starts_at_entry(self, cycle_graph):
        g = cycle_graph
        cycles = detect_cycles(g)
        order = cycle_node_order(cycles[0], g.edge_pairs)
        assert order[0] == cycles[0].entry_node

    def test_all_nodes_present(self, cycle_graph):
        g = cycle_graph
        cycles = detect_cycles(g)
        order = cycle_node_order(cycles[0], g.edge_pairs)
...
class TestDetectRouters:
    ...
    def test_no_routers(self, linear_graph):
        routers = detect_routers(linear_graph)
        assert routers == []

    def test_single_router(self, router_graph):
        routers = detect_routers(router_graph)
        assert len(routers) == 1
        assert routers[0].node == "router"

    def test_router_branches(self, router_graph):
        routers = detect_routers(router_graph)
        r = routers[0]
...
class TestAnalyzeGraphTopology:
    ...
    def test_linear_all_regular(self, linear_graph):
        topo = analyze_graph_topology(linear_graph)
        for nt in topo.node_types.values():
            assert nt == NodeType.REGULAR

    def test_cycle_node_types(self, cycle_graph):
        topo = analyze_graph_topology(cycle_graph)
        types = set(topo.node_types.values())
        assert NodeType.CYCLE_ENTRY in types
        assert NodeType.CYCLE_EXIT in types

    def test_router_node_type(self, router_graph):
        topo = analyze_graph_topology(router_graph)
        assert topo.node_types["router"] == NodeType.ROUTER


class TestFindRouterChains:

    def test_no_chains_single_router(self, router_graph):
        topo = analyze_graph_topology(router_graph)
...
class TestDetectCyclesMultiCycle:
    ...
    def test_nested_cycles_both_found(self, nested_cycle_graph):
        cycles = detect_cycles(nested_cycle_graph)
    ...
    def test_nested_shared_node_in_both(self, nested_cycle_graph):
        cycles = detect_cycles(nested_cycle_graph)
    ...
    def test_disjoint_cycles_both_found(self, disjoint_cycles_graph):
        cycles = detect_cycles(disjoint_cycles_graph)
    ...
    def test_overlapping_cycles_both_found(self, overlapping_cycles_graph):
        cycles = detect_cycles(overlapping_cycles_graph)
        assert len(cycles) >= 2, f"Expected >=2 overlapping cycles, got {len(cycles)}"
        all_back_edges = {c.back_edge for c in cycles}
        assert len(all_back_edges) >= 2

    def test_single_cycle_regression(self, cycle_graph):
        cycles = detect_cycles(cycle_graph)
        assert len(cycles) == 1
        assert len(cycles[0].nodes) == 3

    def test_no_cycles_regression(self, linear_graph):
        cycles = detect_cycles(linear_graph)
        assert cycles == []
...
class TestAnalyzeGraphTopologyMultiCycle:
    ...
    def test_nested_cycle_node_types_priority(self, nested_cycle_graph):
        topo = analyze_graph_topology(nested_cycle_graph)
    ...
    def test_nested_cycle_all_cycle_nodes_sequential(self, nested_cycle_graph):
        topo = analyze_graph_topology(nested_cycle_graph)
        all_sequential = set()
    ...
    def test_overlapping_shared_nodes_classified(self, overlapping_cycles_graph):
        topo = analyze_graph_topology(overlapping_cycles_graph)
...
class TestGetSafeParallelizationGroups:
    ...
    def test_cycle_fallback_deterministic_singletons(self, linear_graph):
    ...
    def test_cycle_fallback_warns(self, caplog, linear_graph):
    ...
    def test_independent_nodes(self, linear_graph):
        topo = analyze_graph_topology(linear_graph)
        deps = {"a": set(), "b": set(), "c": set()}
        groups = get_safe_parallelization_groups(topo, deps)
        assert any(len(g) > 1 for g in groups)

    def test_dependent_nodes(self, linear_graph):
        topo = analyze_graph_topology(linear_graph)
        deps = {"a": set(), "b": {"a"}, "c": {"b"}}
        groups = get_safe_parallelization_groups(topo, deps)
        assert all(len(g) == 1 for g in groups)
